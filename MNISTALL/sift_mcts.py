...
"""
Generate adversarial examples with HTS
...
def sift_apply_direct(model, image):
    ...
    path0 = "%s/%s_org_as_%s_withConf_%s.png"%("image", originalClass, originalClass, originalConfident)
    path1 = "%s/%s_org_as_%s_withConf_%s.png"%("image", originalClass, newClass, newConfident)
    saveImg(-1, image, path0)
    saveImg(-1, image1, path1)
...
def sift_mcts_single(model, image):
    ...
    if re == True:     
         ...
         #path0 = "%s/%s_org_as_%s_withConf_%s.png"%("image", originalClass, originalClass, originalConfident)
         #path1 = "%s/%s_org_as_%s_withConf_%s.png"%("image", originalClass, newClass, newConfident)
         path0 = "%s/%s_org_as_%s_withConf_%s_EU%s_L1%s_L0%s_per%s.png"%("image", originalClass, originalClass, originalConfident, eudist, l1dist, l0dist, percent)
         path1 = "%s/%s_org_as_%s_withConf_%s_EU%s_L1%s_L0%s_per%s.png"%("image", originalClass, newClass, newConfident, eudist, l1dist, l0dist, percent)
         saveImg(-1, image, path0)
         saveImg(-1, image1, path1)
         ...
         #return sift_apply_direct(model, image)
